...
table1 <- bordeaux_cleaned %>%
  dplyr::select(age_group, female, everything()) %>%
  set_variable_labels(
    female = "Women",
    age_group = "Age group",
    rank = "Question order",
...
load(here::here("data", "analysis_data", "Calishare", "wtd_kappa_02022024.RData"))
select <- dplyr::select
...
health_model <- crosswalk_model$ordinal_health
health_model$call$formula <- 
  as.formula("srhealth_share_3cat ~ srhealth_memento")
health_tidy <- 
  broom::tidy(health_model) %>%
  mutate(
    label = str_replace_all(
      term, 
      c("srhealth_memento" = "Continuous measure",
        # "female" = "Female",
        # "age_group3" = "Age 70-79",
        # "age_group4" = "Age: 80-89",
        # "(.*)boundary_knots\\)1" = "Spline term 1",
        ...
        # "(.*)boundary_knots\\)3" = "Spline term 3",
        # "(.*)boundary_knots\\)4" = "Spline term 4",
        "1\\|2" = "Intercept: Excellent/Very good vs. Good, Fair/poor",
        "2\\|3" = "Intercept: Excellent/Very good, Good vs. Fair/poor"))) %>%
  select(label, estimate, std.error) %>%
  set_colnames(c("Coefficient", "Beta", "SE")) %>%
  mutate_if(is.numeric, function(x) sprintf("%.2f", x))

writexl::write_xlsx(list("srhealth" = health_tidy, "srmemory" = memory_tidy),
                    path = here::here("output", "bordeaux_crosswalk",
                                      "tableS1_model_coefficients_03252024.xlsx"))
...
#---- Table S3. kappa for percentile crosswalk ----
...
health_resid <- 
  resids(crosswalk_model[["ordinal_health"]]) %>%
  as_tibble() %>%
  set_colnames("resid")
health_resid_aug <- cbind(bordeaux_cleaned, health_resid)
health_resid_plot <- health_resid_aug %>%
  ggplot() +
  geom_point(aes(x = srhealth_memento, y = resid)) +
